---
description: Adversarial, evidence-driven orchestration for executing, evaluating, or dry-running one DIP or GP phase build.
argument-hint: <invocation YAML or phase-build request>
---
...
Phase text is resolved only from `eng-design/sequential build/`. The companion
architecture reference and the superseded monolith are inputs only and must
never resolve or override a phase.

Processes Activated entries resolve by **normalized process name**.
**Numbers are phase-local** and are not stable across documents, so a number
match corroborates a name match but never substitutes for one. Multiple name
matches are BLOCKED until an operator disambiguates them.
