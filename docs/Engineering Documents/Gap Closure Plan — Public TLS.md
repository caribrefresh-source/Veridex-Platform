...
| 3b | 2026-09-18 | **Stage D splits in two, decided by the repository owner.** **D1** applies the netcup provider firewall (public ports, filtered at the hypervisor); **D2** is the Cilium host policy as already drafted. D1 first, because it closes the NodePort bypass at a layer where a wrong rule is fixed by an API call rather than by console access that is still unproven — the provider firewall never reaches the node's eBPF datapath. Rationale, the committed ruleset and the read-only drift verifier: `docs/security/provider-firewall.md`. The console/rescue drill remains the prerequisite for Stage E regardless; D1 reduces the chance of needing it, not the need to have it. |
