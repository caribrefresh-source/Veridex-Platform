...
| Stage C exit gates | EG95/EG96/EG99/EG100 **PASS**; EG97/EG98 **PARTIAL** | `stage-c/eg95-eg100.txt`. The Traefik Deployment was removed at merge `1712f69`, completing the two-step cutover the DaemonSet header promised. Across the prune: 19 samples at 10s, every probe (Argo CD 32537 and Grafana 32538 on both worker IPs, plus the site on 443) returned 200, no degraded sample. EG100: 14 samples at 35s spanning several auto-sync cycles -- `Synced` 14/14, `outOfSyncResources=0`, `restarts=0,0`, and one distinct pod UID set, identical before and after, so removing the Deployment churned nothing. EG97/EG98 are marked PARTIAL deliberately: as written they require probes during the **2026-09-17 step-1** cutover, a window that closed unobserved and cannot be reconstructed. The step-2 measurement is a substitution, not a satisfaction, of that gate. |

| Stage D host policy (audit mode) | **EG102/EG103/EG104 PASS** 2026-09-19 | `stage-d/eg102-eg104.txt`. PR #93 merged 02:28:58Z; Argo reconciled at `f97a92a`, both CCNPs created 02:30:40Z; all five host endpoints held `PolicyAuditMode: Enabled` / `(Audit)` throughout, so nothing was dropped. EG102 passes under a **source-aware** criterion -- zero legitimate flows would drop across 45 minutes; the 31 audits were all `reserved:world` probing closed ports. Two earlier criteria were wrong and are recorded as rejected, because a naive port-only test reported false failures twice. EG103: 200s on site/Argo CD/Grafana from both workers throughout, 5/5 nodes Ready, 9/9 apps, 0 agent restarts. EG104: a bare `kubectl delete` was undone by selfHeal in **2 seconds**, verifying the header warning; and the recreate produced a **transient** would-be drop of legitimate VXLAN, which under enforcement would be a brief cross-node partition. |
| Stage D prerequisites | **2 of 3 NOT met** | EG101 was met and re-verified. The netcup snapshot was never taken and the five SSH sessions were never established -- the merge proceeded on explicit repeated instruction after the gap was raised three times. No cost was incurred (audit mode held, zero restarts) -- but that was luck, not the safety design: every mitigation the procedure required for exactly this merge was absent. The accepted exposure is recorded rather than omitted. |
| Stage D limitations | Recorded, not blocking | No Ansible run occurred in-window, so EG102 passes on organic traffic only (no SSH access from either party). The cold-apply path Stage E will actually use is unmeasured. Recovery is not claimed. Audit mode itself remains imperative and undocumented -- it should become declarative or an expiring §4 emergency mutation before Stage E. |
