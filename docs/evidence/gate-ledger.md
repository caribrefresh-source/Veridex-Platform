...
Last gate deliverable **D62**, last gate exit-gate check **EG67** (Gate 11,
closed 2026-09-16).

**Numbers allocated outside the gates.** The Public TLS gap-closure plan
(`docs/Engineering Documents/Gap Closure Plan — Public TLS.md`, Revision 3)
uses this same cumulative sequence, confirmed by the repository owner (its
decision R1, 2026-09-16). It holds **D63–D104** and **EG68–EG108**, some built
and some only allocated; see its §0 table. Those numbers are spent whether or
not they are ever built. The next closed gate therefore starts at **D105** and
**EG109**. If that plan allocates more numbers before the next gate closes, move
this start point past them. When that plan completes, record its final range
here.
...
| O5 | 2026-09-17 | Plan: "Resolve the §6 conflict below (SOPS + age vs. SealedSecrets and Longhorn) before Gate 13 or Gate 24 close", and Gate 17 requires a proven Velero row. But the plan has no §6, and `.claude/CLAUDE.md` §6 (Approved Stack) lists SOPS + age while listing neither Sealed Secrets, Longhorn nor Velero. `docs/architecture/proposed-gitops-structure.md` already says "No Velero". Neither mechanism was deployed when this was decided: no `.sops.yaml`, no SOPS-encrypted file and no SealedSecret on main. | **Longhorn: approved**, scoped exactly as the plan scopes it (Gates 12, 26, 27): replicated block storage for suitable PVCs, never MinIO data. **Secrets: SOPS + age only; Sealed Secrets is not adopted.** Reasons: (1) one mechanism covers both owning layers. Host secrets rendered by Ansible (such as the B2 credentials in `roles/etcd-s3-backup`) cannot use Sealed Secrets, which only decrypts inside the cluster, so adopting it would mean running two mechanisms. (2) Recovery: the plan escrows "SOPS age" material in `veridex-recovery-keys` (Gate 18). An escrowed age key decrypts Git on a rebuilt cluster (Gate 29). Sealed Secrets would first need its controller's private key restored into a live cluster, which adds a step to the recovery order. (3) The plan's Gate 21 already says "Keys are encrypted through SOPS in Git". Implementation cost, carried to Gates 21/22: Argo CD needs a decryption path, planned as a KSOPS config-management-plugin sidecar on the repo-server, with the age private key mounted only there. Bootstrap secrets stay Ansible-owned (Gate 10). **Velero: not adopted.** Every data set already has one native restore path: Git plus etcd snapshots for API objects, Longhorn backupstore for Longhorn PVCs, Barman Cloud for CNPG, and replication for MinIO. Velero would give PVC data a second, overlapping restore mechanism, which CLAUDE.md §9 forbids, and would add load to a capacity-constrained cluster. Gate 17's Velero row is withdrawn by plan amendment. Revisit only if a workload appears whose data has no native backup path. | Repository owner, 2026-09-17 session, delegating the call to best-practice advice ("advise and use best practices to resolve") and adopting the resolution proposed there |
