---
name: phase-brief
description: Generate or refresh the executable specification brief for exactly one Document Intelligence Platform phase without implementation or cluster access. Use when the user invokes phase-brief or asks to draft, regenerate, or refresh a phase brief.
---

Resolve exactly one phase identifier from the invoking user request. Interpret every `$1` placeholder below as that validated identifier. If no safe, unique identifier is supplied, stop and ask for it without writing files.
